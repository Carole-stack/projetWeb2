...
function create({ usermail, password }, callback) {
  ...
  bcrypt.hash(password, 10, (err, encryptedPasswordHash) => {
    ...
    const query = "INSERT INTO users (usermail, encrypted_password) VALUES ($1, $2) RETURNING *";
    utils.executeQuery(query, [usermail, encryptedPasswordHash], (err, result) => {
      if (err) {
        callback(true, err);
      } else {
...
function authenticate({ usermail, password }, callback) {
  
  // Execution des commandes SQL
  // On verifie cherche si le user exist
  utils.executeQuery("SELECT * FROM USERS WHERE usermail=$1", [usermail], (err, result) => {
    if (err) {
      callback(true, err);
    } 
